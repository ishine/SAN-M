...
class Decoder(nn.Module):
    ...
    def preprocess(self, padded_input):
        ys = [ flow.Tensor(y[y != IGNORE_ID],dtype=padded_input.dtype).to(padded_input.device) for y in padded_input.numpy()]
        #eos = ys[0].new([self.eos_id])
        #sos = ys[0].new([self.sos_id])

        eos = flow.Tensor([self.eos_id],dtype=ys[0].dtype).to(padded_input.device)
        sos = flow.Tensor([self.sos_id],dtype=ys[0].dtype).to(padded_input.device)
